...
TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/xi-ai-codex-setup.XXXXXX")
cleanup() {
  rm -rf -- "$TEMP_DIR"
}
trap cleanup EXIT
...
IFS= read -r VERSION < "$TEMP_DIR/version.txt"
"$PYTHON_BIN" "$TEMP_DIR/xi-ai-codex-bootstrap.py" \
